package com.feisuo.sds.base.server.arg;
...
import com.feisuo.sds.base.server.annotation.FormModel;
import com.feisuo.sds.base.server.annotation.QueryPage;
import com.feisuo.sds.base.share.util.MapWapper;
import com.feisuo.sds.base.share.util.QueryBeanUtil;
...
public class FormModelMethodArgumentResolver implements
	...
	public boolean supportsParameter(MethodParameter parameter) {
		if (parameter.hasParameterAnnotation(FormModel.class)
				|| parameter.hasParameterAnnotation(QueryPage.class)) {
			return true;
		}
		return false;
	}
	...
	public final Object resolveArgument(MethodParameter parameter,
		...
		if(parameter.getParameterAnnotation(FormModel.class)!=null){
			...
			if (target != null) {
				...
				if (binder.getBindingResult().hasErrors()) {
					if (isBindExceptionRequired(binder, parameter)) {
						...
						List<ObjectError> objectErrors = bindingResult
								.getAllErrors();
					...
					if (component != null) {
						...
						if (componentBinder.getBindingResult().hasErrors()) {
							if (isBindExceptionRequired(componentBinder,
									parameter)) {
								BindingResult bindingResult = binder
										.getBindingResult();
								List<ObjectError> objectErrors = bindingResult
										.getAllErrors();
								StringBuilder error = new StringBuilder();
	...
	private ServletRequest prepareServletRequest(Object target,
		...
		if (StringUtils.isEmpty(modelPrefixName)) {
			modelPrefixName = parameter.getParameterName();
		}
		HttpServletRequest nativeRequest = (HttpServletRequest) request
				.getNativeRequest();
		...
		ParameterRequestWrapper mockRequest = null;
		if (multipartRequest != null) {
			ParameterRequestWrapper mockMultipartRequest = new ParameterRequestWrapper(
					nativeRequest);
			// mockMultipartRequest.getMultiFileMap().putAll(
			// multipartRequest.getMultiFileMap());
		} else {
			mockRequest = new ParameterRequestWrapper(nativeRequest);
		}
	...
	protected void validateComponent(WebDataBinder binder,
		...
		for (Annotation annot : annotations) {
			if (annot.annotationType().getSimpleName().startsWith("FormModel")
					&& validateParameter) {
