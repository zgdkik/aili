...
//        dataType: 'json',
        ...
        formatRecordsPerPage: function (pageNum) {
            return sprintf('%s 每页记录数', pageNum);
        },
        formatShowingRows: function (pageFrom, pageTo, totalRows) {
            return sprintf('显示 %s 到  %s 总  %s 条记录', pageFrom, pageTo, totalRows);
        },
    ...
    BootstrapTable.COLUMN_DEFAULTS = {
        ...
        switchable: true,
        clickToSelect: true,
        formatter: function(val,obj,num){
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        for (i = from; i <= to; i++) {
            html.push('<li class="page-number' + (i === this.options.pageNum ? ' active disabled' : '') + '">',
        ...
        $number.off('click').on('click', $.proxy(this.onpageNum, this));
    };
    ...
    BootstrapTable.prototype.updatePagination = function (event) {
        ...
        this.trigger('page-change', this.options.pageSize, this.options.pageNum);
    };
    ...
    BootstrapTable.prototype.onpageNum = function (event) {
                ...
                if (that.options.columns[j].checkbox || that.options.columns[j].radio) {
                    ...
                    var td ="<td ";
                	 if (value != "" && value != null) {
                		 //  acronyms:false,
                	      //  acronymsLength:30,
						if (that.options.columns[j].acronyms && that.options.columns[j].acronymsLength < value.length) {
							aval = value.substr(0,that.options.columns[j].acronymsLength)+"...";
							td = td+" title='"+value+"'";
						}
					  }
                    text = that.options.cardView ?
                        ['<div class="card-view">',
                            ...
                            sprintf('<span class="value">%s</span>', aval),
                            '</div>'].join('') :
                    ...
                    if (that.options.cardView && that.options.smartDisplay && aval === '') {
                        text = '';
                    }
                }
    ...
    BootstrapTable.prototype.initServer = function (silent) {
        var that = this,
            data = {},
            params = {
                'page.pageSize': this.options.pageSize,
                'page.pageNum': this.options.pageNum,
