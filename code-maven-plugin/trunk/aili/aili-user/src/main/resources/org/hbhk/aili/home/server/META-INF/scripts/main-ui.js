...
(function($) {
	...
	$(document).ready(function() {
		var selects = $(".easyui-combobox");
		if(selects!=null && selects.length>0){
			for (var i = 0; i < selects.length; i++) {
				var combo  = $(selects[i]);
				var select  =combo.attr("data-selected");
				if(typeof(select)!="undefined"){
					combo.combobox("setValue",select)
				}
			}
		}
...
//			$(".textbox").css("width","120px");
	...
	$.extend($.fn.datagrid.methods, {
        statistics: function (jq) {
            ...
            footer['sum'] = "";
            for (var i = 0; opt!=null && opt.length>0 && i < opt[0].length; i++) {
                if (opt[0][i].sum) {
            ...
            if (footer['sum'] != "") {
                ...
                if (obj[opt[0][0].field] == undefined) {
                    footer['sum'] += '"' + opt[0][0].field + '":"<b>单页合计:</b>"';
                    obj = eval('({' + footer['sum'] + '})');
                } else {
                    obj[opt[0][0].field] = "<b>单页合计:</b>" + obj[opt[0][0].field];
                }
                footerObj.push(obj);
            }
            ...
            function sum(filed) {
                    ...
                    for (var i = 0; i < rows.length; i++) {
                    	sumNum = Math.round((sumNum+rows[i][filed])*100)/100;
