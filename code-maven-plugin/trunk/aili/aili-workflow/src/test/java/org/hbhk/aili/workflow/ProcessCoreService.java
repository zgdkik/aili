...
import org.hbhk.aili.workflow.server.service.IProcessCoreService;
import org.hbhk.aili.workflow.server.service.IProcessOtherService;
import org.springframework.beans.factory.annotation.Autowired;

public class ProcessCoreService implements IProcessCoreService {
	@Autowired
	...
	@Autowired
	protected IProcessOtherService processOtherService;
	...
	public List<ActivityImpl> findBackAvtivity(String taskId) {
		...
		if (processOtherService.isJointTask(taskId)) {
			// 会签任务节点，不允许驳回
	...
	public void jointProcess(String taskId, List<String> userCodes)
		...
		for (String userCode : userCodes) {
			String taskIdStr = new Double(1000000 * Math.random()).longValue()
					+ "";
			;
			TaskEntity task = (TaskEntity) taskService.newTask(taskIdStr);
