...
public class AppTest {
	...
	public void test() {
		try {
			...
			List<UserInfo> user = userService.query(params);

			userService.insert(t);
			user = userService.query(params);
			userService.insert(t);
			user = userService.query(params);
			userService.insert(t);
			user = userService.query(params);
	...
	public void getPagetest1() {
		try {
			...
			Pagination<UserInfo> user = userDao.getPagination(params, page,
					new Sort("aaa"));
			System.out.println(user.getDataList());
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//					page, new Sort("name"));
//			if (user != null) {
//				for (UserInfo1 iterable_element : user.getDataList()) {
//					System.out.println("id:" + iterable_element.getName());
//				}
//			}
