...
public class SyncRoleRequestTrans implements IMessageTransform<SyncRoleRequest> {

	/** The Constant CLZZ. */
	private static final Class<SyncRoleRequest> CLZZ =  SyncRoleRequest.class;

	/** The log. */
	private static Log log = LogFactory.getLog(SyncRoleRequestTrans.class);
	
	/** The context. */
	private static JAXBContext context = JAXBContextUtil.initContext(CLZZ);;
	
	@Override
	public SyncRoleRequest toMessage(String string) throws ConvertException, UnsupportedEncodingException {
		if(context == null){
		...
		try {
			...
			JAXBElement<SyncRoleRequest> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	...
	@Override
	public String fromMessage(SyncRoleRequest value) throws ConvertException {
		if(context == null){
			JAXBContextUtil.initContext(CLZZ);//再次尝试一次
			if (context == null) {
				throw new ConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SyncRoleRequest> element = new ObjectFactory().createSyncRoleRequest(value);
