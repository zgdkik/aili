...
public class SyncRoleResponseTrans implements IMessageTransform<SyncRoleResponse> {
	/** The Constant CLZZ. */
	private static final Class<SyncRoleResponse> CLZZ =  SyncRoleResponse.class;

	/** The log. */
	private static Log log = LogFactory.getLog(SyncRoleResponseTrans.class);
	
	/** The context. */
	private static JAXBContext context = JAXBContextUtil.initContext(CLZZ);;
	
	@Override
	public SyncRoleResponse toMessage(String string) throws ConvertException, UnsupportedEncodingException {
		if(context == null){
		...
		try {
			...
			JAXBElement<SyncRoleResponse> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	...
	@Override
	public String fromMessage(SyncRoleResponse value) throws ConvertException {
		if(context == null){
			JAXBContextUtil.initContext(CLZZ);//再次尝试一次
			if (context == null) {
				throw new ConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SyncRoleResponse> element = new ObjectFactory().createSyncRoleResponse(value);
	...
	public static void main(String[] args) throws ConvertException {
		SyncRoleResponse value = new SyncRoleResponse();
		value.setFailCount(1);;
		String ss=  new SyncRoleResponseTrans().fromMessage(value);
