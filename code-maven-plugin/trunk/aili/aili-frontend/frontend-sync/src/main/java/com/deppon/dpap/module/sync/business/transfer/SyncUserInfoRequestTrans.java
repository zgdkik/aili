...
public class SyncUserInfoRequestTrans implements IMessageTransform<SyncUserInfoRequest> {

	/** The Constant CLZZ. */
	private static final Class<SyncUserInfoRequest> CLZZ =  SyncUserInfoRequest.class;

	/** The log. */
	private static Log log = LogFactory.getLog(SyncUserInfoRequestTrans.class);
	
	/** The context. */
	private static JAXBContext context = JAXBContextUtil.initContext(CLZZ);;
	
	@Override
	public SyncUserInfoRequest toMessage(String string) throws ConvertException, UnsupportedEncodingException {
		if(context == null){
		...
		try {
			...
			JAXBElement<SyncUserInfoRequest> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
	...
	@Override
	public String fromMessage(SyncUserInfoRequest value) throws ConvertException {
		if(context == null){
			JAXBContextUtil.initContext(CLZZ);//再次尝试一次
			if (context == null) {
				throw new ConvertException("初始化JAXBContext失败！");
		...
		try {
			...
			JAXBElement<SyncUserInfoRequest> element = new ObjectFactory().createSyncUserInfoRequest(value);
