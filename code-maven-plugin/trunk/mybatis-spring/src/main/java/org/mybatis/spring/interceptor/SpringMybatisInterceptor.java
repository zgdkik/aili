package org.mybatis.spring.interceptor;
...
import org.mybatis.spring.entity.BaseEntity;
import org.mybatis.spring.interceptor.OffsetLimitInterceptor.BoundSqlSqlSource;
import org.mybatis.spring.support.GnericInterfaceTypeContext;
import org.mybatis.spring.support.Pagination;
import org.mybatis.spring.util.FieldColumn;
...
@Intercepts({ @Signature(type = Executor.class, method = "query", args = {
		...
		ResultHandler.class }) })
public class SpringMybatisInterceptor implements Interceptor {
	...
	private MappedStatement copyFromMappedStatement(MappedStatement ms,
		...
		if(dealmethod.contains(methodName)){
			...
			if(id.indexOf("!") < 0){
				if(type.isAssignableFrom(BaseEntity.class) || type.isAssignableFrom(Pagination.class)){
					//处理属性和列名不一样的
					List<ResultMapping> resultMappings= getResultMapping(gnericInterfaceType, ms);
					ResultMap.Builder reBuilder = new ResultMap.Builder(
							ms.getConfiguration(), resultMap.getId(), gnericInterfaceType,
