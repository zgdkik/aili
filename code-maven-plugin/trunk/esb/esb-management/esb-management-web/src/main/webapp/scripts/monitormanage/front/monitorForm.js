...
Ext.define('monitor_thresholdForm', {
	...
	items : [{
				...
				handler : function() {
					...
					if (!form.isValid()) {
						Ext.MessageBox.alert('', '请检查输入是否正确');
					} else {
						Ext.Ajax.request({
...
var querySvcpointForm = Ext.create('Ext.form.Panel', {
	...
	items : [{
				...
				handler : function() {
					...
					Ext.data.StoreManager.lookup('fullThresholdStore').load({
								params : {
...
// 打开前端服务界面
function queryServce() {
	var provider = Ext.getCmp('monitor_thresholdForm').getForm()
			.findField('serviceProvider1').getValue();
	if (provider == null) {
	...
	Ext.data.StoreManager.lookup('monitor_svcpointStore').load({
				params : {
