...
Ext.define('LogQueryForm', {
			...
			items : [{
				...
				fieldLabel : 'esbServiceCode',
				name:'esbServiceCode'
			}, {
				fieldLabel : 'backServiceCode',
				name:'backServiceCode'
			}, {
				fieldLabel : 'requestId',
				name:'requestId'
			}, {
				fieldLabel : 'responseId',
				name:'responseId'
			}, {
				fieldLabel : 'businessId',
				name:'businessId'
			}, {
				fieldLabel : 'sourceSystem',
				name:'sourceSystem'
			}, {
				...
				width : 60,
				handler : function() {
					var store = Ext.data.StoreManager.lookup('Esb2_AuditLog_Store_Id');
							store.loadPage(1, {
								callback : function(records, operations,
										success) {
									var rawData = Ext.getCmp('Esb2_AuditLog_Grid_Id').store.proxy.reader.rawData;
									if (!success) {
...
Ext.define('AuditLogStore', {
			...
			proxy : {
				...
				url : '../audit/queryAuditLogList.action',
				reader : {
					type : 'json',
					root : 'list',
...
Ext.define('ListPanel', {
	...
	columns : [{
				...
				renderer : function(value, celldata, record) {
					...
					return " <button type='button' id='save' onclick='showAuditLogBody(\""
							+ id + "\");'>审计日志 </button>";
				}
			}]
});
...
//============函数==================
function showAuditLogBody(id) {
	//var sid = id;
	var windowId = "showAuditBody";
	Ext.create('Ext.window.Window', {
		...
		html : '<iframe src ="../audit/queryAuditLogBody.action?queryBean.fid='
				+ id
				+ '" width="100%" height="100%" frameborder="no" border="0" framespacing="0" align="left"></iframe>'
	}).show();
};
