	...
	tbar : [{
		...
		handler : function() {
			...
			Ext.MessageBox.confirm('确认框', '你确认删除选择的记录？', function(
				...
				if (btn == "yes") {
					Ext.Ajax.request({
								url : '../monitor/deleteThreshold.action',
								params : {
								...
								success : function(response) {
									...
									Ext.data.StoreManager
											.lookup('fullThresholdStore').remove(selection);
								},
		...
		handler : function() {
			...
			var url = '../monitor/updateThreshold.action';
		...
		text : '新增',
		handler : function() {
			openSvcpointForm('../monitor/addThreshold.action');
		}
	}],
	/**
			 * 分页工具栏
...
function openSvcpointForm(url, parm) {
	...
	Ext.getCmp('monitor_thresholdForm').getForm().findField('svcProvdId').setDisabled(true);
	Ext.getCmp('monitor_thresholdForm').getForm().findField('svcProvdId').setValue('ESB');
	if (parm) {
