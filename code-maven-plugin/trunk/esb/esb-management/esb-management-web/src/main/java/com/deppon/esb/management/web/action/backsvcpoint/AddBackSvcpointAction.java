package com.deppon.esb.management.web.action.backsvcpoint;
...
@Controller("addSvcpointAction")
@Scope("prototype")
public class AddBackSvcpointAction {
	/**
	 * 日志
	 */
	protected static final Logger LOGGER = LoggerFactory.getLogger(AddBackSvcpointAction.class);
	...
	public boolean check() {
		//System.out.println(svcPointInfo);
	...
	public String updateFrontSvcpoint(){
		if(check()){
			//前端服务与后端服务已存在
			if(!svcpointRelationService.isExistSvcpointRelation(svcPointRelationInfo)){
				message ="已经存在同样的前后端服务配置关联";
			}
			else{
				this.svcpointService.addSvcpoint(svcPointInfo, svcPointRelationInfo);
				message="前端服务配置保存成功";
				this.success="true";
			}
		}
	...
	public String addRelation(){
		...
		else if(svcpointRelationService.isExistSvcpointRelation(svcPointRelationInfo)){
