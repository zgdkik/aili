...
public class ReportController {
	...
	public String showReport(@PathVariable String reportName,
		...
		// 当为pdf、xls、csv时的附件名
		Date now = new Date();
		mv.addAttribute(ReportView.ATTACHEMT_FILE_NAME_KEY, reportName + "-"
				+ new SimpleDateFormat("yyyyMMddHHmmss").format(now));
		mv.addAttribute("reportName", reportName);
		mv.addAttribute(ReportView.DEFAULT_FORMAT_KEY, format);
		if (reportExporter != null) {
			// 设置参数
			mv.addAttribute(ReportView.EXPORTER_PARAMETERS_KEY, reportExporter
					.getReportData().getParams());
			// 设置列表数据
	...
	public String test() {
		...
		jdbcTemplate
				.execute("insert into t_aili_user(id,name) values('1','hbhk')");
		return "index";

	}

	@RequestMapping(value = { "/restful", "/restful/{str}", "/restful/t/{str}" })
	@ResponseBody
