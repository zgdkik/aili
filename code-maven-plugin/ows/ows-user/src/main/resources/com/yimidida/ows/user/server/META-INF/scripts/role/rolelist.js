$(document).ready(function() {
    ...
    function queryData(){
	    ...
	    ym.initPage(base + "/role/getRoleList",{},params,'#dg',columns);	
    }
	//查询
	...
	$("body").on("click",'.update',function(){
		...
		ym.sendPost(base+'/role/getRoleByid/'+id,null,{successHandler:function(data, textStatus, jqXHR){
			if(data !=null){
				if(data.success){
	...
	function save(){
		...
		ym.sendPost(base + "/role/editRole/",form,{successHandler:function(data, textStatus, jqXHR){
			if(data !=null){
				if(data.success){
					$("#dg").datagrid("reload");
					$('#dlg').dialog('close');
	...
	$('body').on("click",'.delete',function(){
		...
		$.messager.confirm("提示信息","您确定要删除吗",function(data){
			if(data){
				ym.sendPost(base+'/role/deleteRole/'+id,null,{
