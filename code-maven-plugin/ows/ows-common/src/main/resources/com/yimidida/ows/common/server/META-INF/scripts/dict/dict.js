$(document).ready(function() {
	...
	$("#saveDict").on("click",function(){
		...
		ym.sendPost(base+"/dict/addDict/",param,{successHandler:function(data, textStatus, jqXHR){
			if (data != null) {
				if(data.success){
	...
	$("#updateDict").on("click",function(){
		...
		//请求修改的数据
		ym.sendPost(base+"/dict/showEditDictForm/"+dictCode,null,{successHandler:function(data, textStatus, jqXHR){
			if (data != null) {
				if(data.success){
					if (data != null) {
						ym.setFormObj("dictUpdate-form",data.data);
	...
	$("#saveUpdateDict").on("click",function(){
		...
		ym.sendPost(base+"/dict/editDict/",param,{successHandler:function(data, textStatus, jqXHR){
			if (data != null) {
				if(data.success){
					//成功之后自己组装节点，然后增加的父节点下面
	...
	$("#deleteDict").on("click",function(){
		...
		var dictCode=treeNode.id;
		//删除
		ym.sendPost(base+"/dict/deleteDict/"+dictCode,null,{successHandler:function(data, textStatus, jqXHR){
			if (data != null) {
				if(data.success){
					remove();
				}else if(data.success===undefined){
	...
	//查询数据字典
	 function queryData(treeId){
    	ym.initPage(base + "/dict/getDictValueList", {
	...
	$("#saveDictValue").on("click",function(){
		...
		ym.sendPost(base+"/dict/addDictValue/",param,{successHandler:function(data, textStatus, jqXHR){
			if (data != null) {
				if(data.success){
					//成功之后自己组装节点，然后增加的父节点下面
	...
	$('#dictValueTable').on("click",".update",function(){
		...
		//请求修改的数据
		ym.sendPost(base+"/dict/showEditDictValueForm/"+id,null,{successHandler:function(data, textStatus, jqXHR){
			if (data != null) {
				if(data.success){
					if (data != null) {
						ym.setFormObj("dictValueUpdate-form",data.data);
	...
	$("#updateDictValue").on("click",function(){
		...
		ym.sendPost(base+"/dict/updateDictValue/",param,{successHandler:function(data, textStatus, jqXHR){
			if (data != null) {
				if(data.success){
					//成功之后自己组装节点，然后增加的父节点下面
	...
	$('#dictValueTable').on("click",".delete",function(){
		...
		bootbox.confirm("确定要删除吗?", function(result) {
			if(result){
				ym.sendPost(base+'/dict/deleteDictValue/'+id,null,{
