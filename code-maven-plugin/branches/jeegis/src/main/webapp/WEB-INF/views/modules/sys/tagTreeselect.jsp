...
<head>
	...
	<script type="text/javascript">
		...
		var url ="${ctx}${url}${fn:indexOf(url,'?')==-1?'?':'&'}&extId=${extId}&isAll=${isAll}&module=${module}&t=";

		function getDynamicData(val){
			$.get(url+ new Date().getTime()+"&name="+val, function(zNodes1){
				// 初始化树结构
				tree = $.fn.zTree.init($("#tree"), setting, zNodes1);
			...
			key = $("#key");
			if(decodeURIComponent(window.location.search).indexOf("isDynamic")<0){
		...
		$(document).ready(function(){
			$.get(url+ new Date().getTime(), function(zNodes){
				// 初始化树结构
