...
if(typeof(aili) =="undefined"){
	aili ={};
}
aili.empSelector=function(empId){
	setTimeout(function(){
		var emp = $(empId);
		emp.combobox({
			url : base+'/emp/selector',
			textField : 'empName',
			valueField : 'empCode',
			mode : 'remote',
			panelHeight : '150',
...
aili.deptSelector=function(deptId){
	setTimeout(function(){
		var emp = $(deptId);
		emp.combobox({
			url : base+'/dept/selector',
			textField : 'deptName',
			valueField : 'deptCode',
			mode : 'remote',
			panelHeight : '150',
