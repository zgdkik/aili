...
import org.hbhk.aili.base.share.util.CheckMobileUtil;
import org.hbhk.aili.base.share.util.MD5Util;
...
import org.hbhk.hls.user.server.dao.ILoginLogDao;
import org.hbhk.hls.user.server.dao.IUserDao;
import org.hbhk.hls.user.server.service.ILoginService;
import org.hbhk.hls.user.server.service.IUserRoleService;
import org.hbhk.hls.user.server.service.IUserService;
import org.hbhk.hls.user.share.entity.LoginLogEntity;
...
public class LoginService implements ILoginService {
	...
	public UserEntity login(String userName, String password,
		...
		IUser userCache = CacheManager.getInstance().getCache(
				FrontendConstants.USER_CACHE_UUID, userName);
		if (userCache == null) {
		...
		CookieUtil.setCookie(response, AppConst.SESSION_COOKIE_KYE,
				token.getUserCode());
	...
	public void changePassword(String userName, String password,
			String newPassword, String captcha, String type) {
		...
		if ("retrieve".equals(type)) {
			...
			// 验证码有效时间半个小时
			String serverCaptcha = (String) cacheTemplet.get(captcha_key
					+ userName);
		...
		// 成功后清除缓存数据
		if ("retrieve".equals(type)) {
			String[] keys = new String[] { captcha_key + userName,
					password_error_count_key + userName,
					password_error_count_key + userName };
	...
	public String getCaptcha(String userName, String phone) {
		...
		// }
		// Integer num = (Integer)
		// cacheTemplet.get(captcha_key_day_count+userName);
		// if(num!=null && num>5){
