...
public class PropertiesHelper {
	...
	/**
	 * 果在保存属性列表时发生 I/O 错误，则此方法不抛出 IOException。 保存属性列表的首选方法是通过 store(OutputStream
	 * out, String comments) 方法或 storeToXML(OutputStream os, String comment)
	 * 方法来进行。
	 */
	@Deprecated
	...
	/**
	 * 适合使用 load 方法加载到 Properties 表中的格式， 将此 Properties 表中的属性列表（键和元素对）写入输出流。
	 */
	...
	/**
	 * 返回此 Hashtable 中所包含值的 Collection 视图。 Collection 受 Hashtable 的支持， 所以对
	 * Hashtable 的更改反映在 Collection 中， 反之亦然。Collection 支持元素的删除（它从 Hashtable
	 * 中移除相应的条目)， 但不支持元素的添加。
	 * 
	 */
