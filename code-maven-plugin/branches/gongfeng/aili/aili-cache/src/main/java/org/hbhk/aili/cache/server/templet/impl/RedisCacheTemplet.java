...
public class RedisCacheTemplet<V> implements ICacheTemplet<String, V>,
			...
			boolean success = stringRedisTemplate
					.execute(new RedisCallback<Boolean>() {
						...
						public Boolean doInRedis(RedisConnection connection)
								throws DataAccessException {
							// String valueStr =
							// CacheUtils.toJsonString(newValue);
							byte[] bs = cacheSerialize.serialize(newValue);
							if (connection.exists(newKey.getBytes())) {
								return false;
							}
					...
					.execute(new RedisCallback<Boolean>() {
						...
						public Boolean doInRedis(RedisConnection connection)
							...
							for (int i = 0; i < keyValues.size(); i++) {
								...
								byte[] bs = cacheSerialize.serialize(keyValue
										.getValue());
	...
	@Override
	public void invalid(final String key) {
		if (key == null) {
			log.error("key不允许为null或空串!");
			throw new RuntimeException("key不允许为null或空串!");
		}
...
//		}
	...
	@Override
	public ICacheSerialize<byte[]> getCacheSerialize() {
