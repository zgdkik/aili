...
var columns = [[{
    ...
    function queryData(){
	    ...
	    aili.initPage(base + "/common/getSystemParameterList",{},params,'#dg',columns);	
    }
	//查询
	...
	$("body").on("click",'.update',function(){
		...
		aili.sendPost(base+'/common/showSystemParameterForm',{'id':id},{successHandler:function(data, textStatus, jqXHR){
			if(data !=null){
				if(data.success){ 
	...
	function save(){
		...
		aili.sendPost(base + "/common/addSystemParameter",form,{successHandler:function(data, textStatus, jqXHR){
			if(data !=null){
				if(data.success){
					$("#dg").datagrid("reload");
					$('#dlg').dialog('close');
	...
	$('body').on("click",'.delete',function(){
		...
		$.messager.confirm("提示信息","您确定要删除吗",function(data){
			if(data){
				aili.sendPost(base+ "/common/deleteSystemParameter/" +id,null,{successHandler:function(data, textStatus, jqXHR){
					if(data !=null){
						if(data.success){
							$('#dg').datagrid('reload');
							$.messager.alert("提示信息",data.msg);
