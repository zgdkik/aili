	...
	<script type="text/javascript">
		//rowsId总条数
	 	...
	 	/* $("#a_prev").on('click',function(){
	 		if(preId){
	 			location.href="${base}/ddwlGw/queryNoticeDetail?noticeId="+preId+"&rowsId="+rowsId;
	 		}else{
	 			alert("没有更多数据");
	 		}
	 	...
	 	$("#a_next").on('click',function(){
	 		if(nextId){
	 			location.href="${base}/ddwlGw/queryNoticeDetail?noticeId="+nextId+"&rowsId="+rowsId;
	 		}else{
	 			alert("没有更多数据");
	 		}
	 	}); */
	 	/* $("#returnList").on('click',function(){
	 		location.href="${base}/ddwlGw/notice?noticeType=1&compCode=ddwl";
	 	});
	 	 $.ajax({
		        type: "POST",
		 	...
		 	$("#a_next").on('click',function(){
		 	 ...
		 	 $.ajax({
			        ...
			        success: function(data){
			        	...
			        	$("#noticeTitle").html(data.data.noticeTitle);
			        	$("#noticeDate span").html(ym.formatDate(data.data.releaseTime));
			        	replaceSrc();
			        }
			    });
		})
